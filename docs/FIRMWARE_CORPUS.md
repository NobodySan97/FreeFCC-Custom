...
## Сетевой набор `dji_firmware` (2026-07-25)

Появился отдельный каталог на SMB-шаре (`//192.168.1.100/Toshiba_6TB`, на
`homesrv` — `/mnt/toshiba_6tb/`), смонтирован локально в `/tmp/srv`. Анализ
ведётся по месту, без копирования в проект.

| Пакет | Размер | SHA-256 | Что дал |
|---|---:|---|---|
| `dji_firmware/V03.02.0700_rm510_dji_system.bin` | 1 500 999 680 | `978e3f005e1236c29067d2112fc63abbd5aa2a2446faccdf2b87889607b74bb9` | модуль `0600` (`type=GD32`, `enc_key` пуст) — третья незашифрованная MCU-прошивка пульта |
| `dji_firmware/V02.02.0803_rm700_dji_system.bin` | 1 475 389 440 | `a7d15d49d8ebe9968cafa9202d2828a626d0664d2d606dc21de776024b840782` | DJI RC Pro; модуль `0600` (`type=GD32`, `enc_key` пуст) — четвёртая незашифрованная MCU-прошивка пульта; новых команд нет, таблица — подмножество `rc520`, а набор `0x06` совпадает с RM510 |
| `dji_firmware/V17.02.0501_wa345t_dji_system.bin` | 665 825 280 | — | 20+ модулей; все проверенные (`0105`, `0501`, `1200`, `1202`) не проходят checksum, то есть зашифрованы |

Распакованный `rm510_0600_0600.bin` — 159 972 байта, SHA-256
`10cae99bcba5a2e61f12eb785c60444e728fcf1ef84cb335e50e7650221b5053`,
`ARM Cortex-M`, база `0x08010000`. Разбор таблицы и вывод про `06:72` —
в [`RC_MCU_DUML_COMMAND_MAP.md`](RC_MCU_DUML_COMMAND_MAP.md).

Распакованный `rm700_0600_0600.bin` — 180 712 байт, SHA-256
`52b4fb719e56c6550e543d2126d5445afe36b1829a730bd7ec2811eb4293fb7a`,
GD32 Cortex-M, SP `0x20016d38`, reset `0x08010631`, база `0x08010000` (как у
RM510), таблица на `0x08038c1c` = file+`0x28c1c` (34 команды, rc520-формат).
Заголовок модуля даёт `date=2023-10-27`, тогда как имя `.sig` и строка образа
`BUILD Oct 24 2023 21:15:44` указывают на 24-е — расхождение самого пакета.
Полный разбор — в [`RM700_RC_PRO_MCU.md`](RM700_RC_PRO_MCU.md).
Модули `1301` (radio «pigeon», под-образы под TBIE) и `0205` (Android A/B OTA)
в этом заходе не распакованы.

Состав `V17.02.0501_wa345t`: `0103` (два варианта, `IA640` и `HK`), `0105`,
`0106`, `0501`, `0802` (291 МБ, `ar0`), `1005`, `1006`, `1100` (три варианта:
`BA03WA345`, `WA345PTL`, `WA345GY0`), `1200`/`1202` (`mc01`, `mc02`), `1502`
(360 МБ, `ar0`), `2400`/`2401`/`2402` (`RD03`). Крупные `0802` и `1502` в этом
заходе не распаковывались.
...
`03:AF`, `06:8C` и эффект `0xffff0063` дальнейшим анализом открытой
