...
| `dji_lte` | **регистрация в рантайме** | **неприменим** |
...
Его таблица `0x18` собирается кодом при старте (`lte_event_uav_start`), поэтому
опубликованная в [`LTE_DUML_COMMAND_REFERENCE.md`](LTE_DUML_COMMAND_REFERENCE.md)
версия остаётся единственным способом её получить; независимой перепроверки
этим приёмом не существует.
...
|---|---:|---|---|
| WA530 `dji_sys` | 222 записи, ни одна не ведёт на `sys_event_*` | нет | таблица собирается в рантайме |
...
образах ни как 8-байтные слова в данных, ни как `r_addend` релокаций. То есть
на дроне регистрация handler'ов — динамическая, как в `dji_lte`, и статической
таблицы, из которой можно прочитать пары, там просто нет.
