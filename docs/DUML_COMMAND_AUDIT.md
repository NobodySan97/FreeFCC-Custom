...
| 4G legacy sweep | abstract Unix `/duss/mb/0x205`, `51:00..7F` | `0x205` — local router; destination `0xEE` = host `0x0e07` = `dji_wlm`. Полный разбор таблицы регистрации (RC2, RC Pro 2 v576, WA530) даёт **6 заполненных слотов: `51:30/31/32/33/35/36`** — device-manager sync линка WLM; остальные 122 ID sweep'а попадают в пустые слоты и не обрабатываются. Слотов, связанных с LTE, в наборе нет. В пользовательском live-тесте sweep не дал видимого эффекта. См. [`WLM_CMDSET_51.md`](WLM_CMDSET_51.md) | Структура `CONFIRMED`; activation не подтверждена, наблюдаемый эффект `NEGATIVE` |
...
шесть слотов (`30`, `31`, `32`, `33`, `35`, `36`, причём варианты таблицы
взаимоисключающие, так что одновременно активны 4 или 5). Верхняя граница
таблицы — `0x36`, поэтому 122 из 128 кадров sweep'а попадают в пустые слоты и
не обрабатываются вовсе; оставшиеся шесть приходят в device-sync handler'ы с
payload `00 00 00 + ASCII S/N`, который для них мусор. При этом реальная
