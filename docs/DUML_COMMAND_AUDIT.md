...
С 2026-07-24 country setup отделён от повторяемого core:

1. одна `07:30=AU` к `dst=0x09`;
2. один read-only `07:19` к тому же destination;
...
выполняется read-only `07:19`. Если он вернул `AU`, `07:30` не отправляется
вообще. При любом другом ответе (включая отсутствие ответа) выполняется
`07:30=AU` и повторный `07:19`; пара повторяется до трёх раз, пока readback не
вернёт `AU`. Неподтверждённый результат логируется как `verified=false,
...
| 4G legacy sweep | abstract Unix `/duss/mb/0x205`, `51:00..7F` | `0x205` — local router; destination `0xEE` = host `0x0e07` = `dji_wlm`. Полный разбор таблицы регистрации (RC2, RC Pro 2 v576, WA530) даёт объединение из **6 handler ID: `51:30/31/32/33/35/36`** — device-manager sync линка WLM. В конкретной конфигурации активны только 4 или 5 из них, поэтому 124 либо 123 кадра sweep'а не имеют handler'а и не обрабатываются. Слотов, связанных с LTE, в наборе нет. В пользовательском live-тесте sweep не дал видимого эффекта. См. [`WLM_CMDSET_51.md`](WLM_CMDSET_51.md) | Структура `CONFIRMED`; activation не подтверждена, наблюдаемый эффект `NEGATIVE` |
...
`18:4B/4C`. Разбор приёмной стороны (2026-07-25, [`WLM_CMDSET_51.md`](WLM_CMDSET_51.md))
показал, что текущий sweep `51:00..7F` не просто не имеет readback: набор `0x51`
у `dji_wlm` — это device manager линка WLM, и в его таблице заполнено ровно
шесть handler ID (`30`, `31`, `32`, `33`, `35`, `36`, причём варианты таблицы
взаимоисключающие, так что одновременно активны 4 или 5). Верхняя граница
таблицы — `0x36`; в конкретном runtime 124 либо 123 кадра из 128 не имеют
активного handler'а и не обрабатываются вовсе. Оставшиеся 4 или 5 приходят в
device-sync handler'ы с payload `00 00 00 + ASCII S/N`, который не
соответствует их контракту. При этом реальная
отправка всего sweep, выполненная пользователем, не дала видимого эффекта.
Поэтому профиль сохраняется только как явно названный legacy research sweep, а
не как доказанная 4G activation.

Прежняя редакция этого раздела утверждала, что таблица «имеет только ID
`00..51`», и трактовала `51:1A`, `51:19`, `51:22`. Это не воспроизводится:
`0x51` — индекс *cmd_set* во внешнем массиве регистрации, а не размер таблицы,
и слотов `0x19`, `0x1A`, `0x22` нет ни в одном из трёх проверенных образов
`dji_wlm` (RC2, RC Pro 2 v576, WA530).
