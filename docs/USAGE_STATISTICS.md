...
Один JSON snapshot `schema_version=2` содержит:
...
- версию DJI Fly (`versionName` и `longVersionCode`) установленного пакета
  `dji.go.v5`;
- последний прочитанный полный `aircraft_serial`; пустая строка означает, что
  приложение ещё не обнаружило S/N дрона;
...
S/N пульта и дрона нормализуются в верхний регистр, но не хешируются. Они
являются постоянными идентификаторами устройств. `installation_id` отличает
переустановки на одном пульте; принимающая сторона может группировать их по
`controller_serial`.

`aircraft_serial` хранится только для текущей подтверждённой модели. При
подтверждённой смене модели прежний S/N удаляется до нового чтения, поэтому
пустое поле также может означать короткий промежуток идентификации нового борта.
Принимающая сторона накапливает все уникальные S/N дронов по полному
`controller_serial`: смена дрона обновляет последнее поле snapshot, но не удаляет
ранее замеченные дроны из истории этого пульта.

## Что не отправляется

- S/N батареи, камеры, подвеса или модема;
...
4. Впервые прочитанный или изменившийся S/N пульта или дрона, а также первое
   появление или изменение пары кода/названия дрона разрешают один внеочередной
   snapshot.
...
6. Появление проверенного сетевого подключения отменяет этот часовой интервал
   для отчёта, который уже просрочен и чья последняя попытка не удалась. Пульт
   включают на один полёт и убирают, поэтому ждать час — значит чаще всего не
   отправить вовсе. Суточный предел это не обходит: он проверяется первым, и
   больше одного snapshot в сутки не отправляется ни при каких условиях.

Клиент сохраняет точный JSON и повторяет один
`installation_id + report_sequence`, пока не получит успешный HTTP 2xx от
каждого настроенного приёмника. До этого sequence не увеличивается и новый body
под тем же sequence не строится. После завершения отложенной доставки следом
уходит актуальный snapshot, если локальное состояние успело измениться.
Принимающий backend обязан обрабатывать точный повтор идемпотентно; его
реализация ведётся приватно.
...
Название дрона читается из accessibility-текста DJI Fly. Изменившаяся модель
должна оставаться видимой не менее секунды; после подтверждения старый S/N
удаляется и разрешается один bounded-запрос S/N на порту `40007` — не более двух
попыток по 600 мс. Повтор разрешается только после реального состояния
disconnect/`N/A` не короче десяти секунд либо после следующей подтверждённой
смены модели. Home Point, таймер и отправка статистики запрос не запускают.

Известный product code сильнее произвольной подписи на том же экране и задаёт
каноническую пару кода/названия. `DJI Lito`, `DJI Simulator`,
`DJI Global Creator Community` и `DJI Inc` отбрасываются как служебные строки;
`DJI Lito X1` остаётся реальной моделью `WA151`.
...
```json
{
  ...
  "aircraft_serial": "1581F6ABCDEF1234",
...
цикле. Недоступность одного не отменяет попытку на другом, но точный JSON
остаётся локально pending и повторяется на все адреса до подтверждения каждым.
Только после этого sequence увеличивается. Совпадающие адреса считаются одним
приёмником и отчёт дважды туда не уходит.
