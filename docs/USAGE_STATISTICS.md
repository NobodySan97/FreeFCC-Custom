...
Android-клиент входит в release-сборки начиная с `v1.5.61`. Отправка работает
только в доверенной сборке, где задан `FREEFCC_STATISTICS_ENDPOINT` или Gradle
property `statisticsEndpoint`; обычная локальная сборка оставляет
`BuildConfig.STATISTICS_ENDPOINT` пустым и не выполняет запросы.
...
4. Впервые прочитанный или изменившийся S/N пульта, а также первое появление или
   изменение пары кода/названия дрона разрешают один внеочередной snapshot.
...
Название дрона по возможности сохраняется дословно из accessibility-текста DJI
Fly. Если при Home Point название или product code ещё не получены, выполняется
одно короткое пассивное чтение уже существующего DUML-потока без отправки команд.
`00:82` считается первичным product code; произвольная UAV user string `03:34`
принимается как название только после фильтрации. Для известного `WA530`
используется название `DJI Avata 360`, а служебная строка `DJI Inc` отбрасывается.
