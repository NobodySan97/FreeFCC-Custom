...
- Модель дрона теперь читается в первую очередь с экрана DJI-приложения: оно
  печатает и product code, и название, поэтому при совпадении DUML-порт не
  открывается вообще. Пассивное чтение `00:82`/`03:34` осталось фолбэком для
  экранов, где модель не написана.
